...
class ODDemandGenerator:
    ...
    def __init__(self, config):
        ...
        if config.simulation_type == "beijing":
            self.mode_split = {
                'private_car': config.private_car_ratio,
                'ride_hailing': config.ridehail_order_ratio,
            }
        else:
            self.mode_split = {
                'private_car': config.private_car_ratio,
    ...
    def _build_graph(self, road_edges: Dict):
        """由net提取的边(from_node,to_node)构建有向图（只用节点层面，轻量）"""
        self._edge_to_nodes = {}  # edge_id -> (from_node, to_node)
        self._adj = defaultdict(list)  # node -> [next_nodes]
        self._nodes = set()

        for eid, info in road_edges.items():
            if eid.startswith(':'):  # 跳过内部边
                continue
            u = info.get('from_node')
            v = info.get('to_node')
            if not u or not v:
                continue
            self._edge_to_nodes[eid] = (u, v)
            self._adj[u].append(v)
            self._nodes.add(u);
            self._nodes.add(v)

    def _reachable_nodes_from(self, start_node: str, max_steps: int = 10000) -> set:
        """简单BFS；限制步数防止极端情况"""
        vis, q = set(), [start_node]
        while q and len(vis) < max_steps:
            x = q.pop(0)
            if x in vis:
                continue
            vis.add(x)
    ...
    def _check_edge_reachability(self, from_edge: str, to_edge: str) -> bool:
        """由“边到边”转换为“to_node 到 from_node”的可达性判断"""
        ...
        if from_edge == to_edge:
            return False
        fn = self._edge_to_nodes.get(from_edge, (None, None))[1]  # from_edge 的 to_node
        tn = self._edge_to_nodes.get(to_edge, (None, None))[0]  # to_edge   的 from_node
        if not fn or not tn:
            return False
        # BFS 判断 fn -> tn 是否可达
        return tn in self._reachable_nodes_from(fn)
    ...
    def _assign_edges_to_zones(self, road_edges: Dict):
        ...
        # 过滤掉内部边
        edges_list = [e for e in edges_list if not e.startswith(':')]
        ...
        print(f"完成边分配：{len(edges_list)}条边分配到{len(self.zones)}个区域")
