...
class EmissionCalculator:
    ...
    def assign_vehicle_type(self, vehicle_id):
        """为车辆分配类型 - 基于车辆ID确定性分配"""
        # 使用车辆ID的哈希值确保相同ID总是得到相同类型
        import hashlib
        hash_val = int(hashlib.md5(vehicle_id.encode()).hexdigest()[:8], 16)
        normalized_hash = (hash_val % 1000) / 1000.0

        if normalized_hash < self.vehicle_distribution['gasoline']:
            return 'gasoline'
        elif normalized_hash < self.vehicle_distribution['gasoline'] + self.vehicle_distribution['diesel']:
            return 'diesel'
        else:
            return 'electric'
